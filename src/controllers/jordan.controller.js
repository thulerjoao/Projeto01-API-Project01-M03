...
const findByIdJordanController = (req, res) => {
    ...
    if(!escolhajordan){
        return res.status(404).send({message: "ID invárido. Jordan não encontrado."})
    }
...
const createJordanController = (req, res) => {
    ...
    if(!jordan
        ...
        return res.status(400).send({message: "Cadastro Inválido. Preencha todos os campos."})
    }
...
const deleteJordanController = (req, res) => {
    ...
    if(vazio == undefined || vazio == ""){
        return res.status(404).send({message: "ID inválido. Jordan não encontrado."})
    }
